...
const fullspecieslist=[:CO2, :O2, :O3, :H2, :OH, :HO2, :H2O, :H2O2, :O, :CO, :O1D, :H,:N2,:Ar,:CO2pl,:HOCO];
...
const speciesmolmasslist = Dict( :CO2=>44, :O2=>32, :O3=>48, :H2=>2, :OH=>9, :HO2=>17, :H2O=>18, :H2O2=>34, :O=>16, :CO=>28, :O1D=>16, :H=>1, :N2=>28, :Ar=>40, :CO2pl=>44, :HOCO=>45)
...
const Jratelist=[:JCO2ion,:JCO2toCOpO,:JCO2toCOpO1D,:JO2toOpO,:JO2toOpO1D,:JO3toO2pO,:JO3toO2pO1D,:JO3toOpOpO,:JH2toHpH,:JOHtoOpH,:JOHtoO1DpH,:JHO2toOHpH,:JH2OtoHpOH,:JH2OtoH2pO1D,:JH2OtoHpHpO,:JH2O2to2OH,:JH2O2toHO2pH,:JH2O2toH2OpO1D];
...
#Yes, this is ugly, but should return to sanity once [ [], [] ] no
#longer automatically concatenates, and the Any[ specifications can be
#removed.
reactionnet=Any[
             #Photodissociation
             Any[[:CO2],[:CO,:O],:JCO2toCOpO],
             Any[[:CO2],[:CO,:O1D],:JCO2toCOpO1D],
             Any[[:O2],[:O,:O],:JO2toOpO],
             Any[[:O2],[:O,:O1D],:JO2toOpO1D],
             Any[[:O3],[:O2,:O],:JO3toO2pO],             
             Any[[:O3],[:O2,:O1D],:JO3toO2pO1D],
             Any[[:O3],[:O,:O,:O],:JO3toOpOpO],             
             Any[[:H2],[:H,:H],:JH2toHpH],
             Any[[:OH],[:O,:H],:JOHtoOpH],
             Any[[:OH],[:O1D,:H],:JOHtoO1DpH],
             Any[[:HO2],[:OH,:O],:JHO2toOHpH],#other branches should be
                                           #here, but have not been
                                           #measured
             Any[[:H2O],[:H,:OH],:JH2OtoHpOH],
             Any[[:H2O],[:H2,:O1D],:JH2OtoH2pO1D],
             Any[[:H2O],[:H,:H,:O],:JH2OtoHpHpO],
             Any[[:H2O2],[:OH,:OH],:JH2O2to2OH],
             Any[[:H2O2],[:HO2,:H],:JH2O2toHO2pH],
             Any[[:H2O2],[:H2O,:O1D],:JH2O2toH2OpO1D],
             
             #recombination of O
             Any[[:O,:O,:M],[:O2,:M],:(1.8*3.0e-33*(300/T)^3.25)],
             Any[[:O,:O2,:N2],[:O3,:N2],:(5e-35*exp(724./T))],
             Any[[:O,:O2,:CO2],[:O3,:CO2],:(2.5*6.0e-34*(300./T)^2.4)],
             Any[[:O,:O3],[:O2,:O2],:(8.0e-12*exp(-2060./T))],
             Any[[:O,:CO,:M],[:CO2,:M],:(2.2e-33*exp(-1780./T))],
             
             #O1D attack
             Any[[:O1D,:O2],[:O,:O2],:(3.2e-11*exp(70./T))],
             Any[[:O1D,:O3],[:O2,:O2],:(1.2e-10)],
             Any[[:O1D,:O3],[:O,:O,:O2],:(1.2e-10)],
             Any[[:O1D,:H2],[:H,:OH],:(1.2e-10)],
             Any[[:O1D,:CO2],[:O,:CO2],:(7.5e-11*exp(115./T))],
             Any[[:O1D,:H2O],[:OH,:OH],:(1.63e-10*exp(60./T))],
             
             #loss of H2
             Any[[:H2,:O],[:OH,:H],:(6.34e-12*exp(-4000/T))],
             Any[[:OH,:H2],[:H2O,:H],:(9.01e-13*exp(-1526/T))],
             
             #recombination of H
             Any[[:H,:H,:CO2],[:H2,:CO2],:(1.6e-32*(298./T)^2.27)],
             Any[[:H,:OH,:CO2],[:H2O,:CO2],:(1.9*6.8e-31*(300./T)^2)],
             Any[[:H,:HO2],[:OH,:OH],:(7.2e-11)],
             Any[[:H,:HO2],[:H2O,:O1D],:(1.6e-12)],#O1D is theoretically mandated
             Any[[:H,:HO2],[:H2,:O2],:(0.5*6.9e-12)],
             Any[[:H,:H2O2],[:HO2,:H2],:(2.8e-12*exp(-1890/T))],
             Any[[:H,:H2O2],[:H2O,:OH],:(1.7e-11*exp(-1800/T))],
             
             #Interconversion of odd H
             Any[[:H,:O2],[:HO2],threebody(:(2.0*4.4e-32*(T/300)^-1.3),
                                        :(7.5e-11*(T/300)^0.2))],
             Any[[:H,:O3],[:OH,:O2],:(1.4e-10*exp(-470/T))],
             Any[[:O,:OH],[:O2,:H],:(1.8e-11*exp(180/T))],
             Any[[:O,:HO2],[:OH,:O2],:(3.0e-11*exp(200./T))],
             Any[[:O,:H2O2],[:OH,:HO2],:(1.4e-12*exp(-2000/T))],
             Any[[:OH,:OH],[:H2O,:O],:(1.8e-12)],
             Any[[:OH,:OH],[:H2O2],threebody(:(1.3*6.9e-31*(T/300)^-1.0),:(2.6e-11))],
             Any[[:OH,:O3],[:HO2,:O2],:(1.7e-12*exp(-940/T))],
             Any[[:OH,:HO2],[:H2O,:O2],:(4.8e-11*exp(250/T))],
             Any[[:OH,:H2O2],[:H2O,:HO2],:(1.8e-12)],
             Any[[:HO2,:O3],[:OH,:O2,:O2],:(1.0e-14*exp(-490/T))],
             Any[[:HO2,:HO2],[:H2O2,:O2],:(3.0e-13*exp(460/T))],
             Any[[:HO2,:HO2,:M],[:H2O2,:O2,:M],:(2*2.1e-33*exp(920/T))],
             
             #CO2 recombination due to odd H (with HOCO intermediate)
             Any[[:CO,:OH],[:CO2,:H],threebodyca(:(1.5e-13*(T/300)^0.6),:(2.1e9*(T/300)^6.1))],
             Any[[:OH,:CO],[:HOCO],threebody(:(5.9e-33*(T/300)^-1.4),:(1.1e-12*(T/300)^1.3))],
             Any[[:HOCO,:O2],[:HO2,:CO2],:(2.0e-12)],
             
             #CO2+ attack on molecular hydrogen
             Any[[:CO2pl,:H2],[:CO2,:H,:H],:(8.7e-10)]
             ]
...
function Tspl(z::Float64,lapserate=-1.4e-5,Tsurf=211,ztropo=50e5,zexo=200e5,Texo=300)
    ...
    Tret=(
          (z - zexo)^2
          *(Ttropo*(2*z + zexo - 3*ztropo) + lapserate*(z - ztropo)*(zexo - ztropo))
          -Texo*(z - ztropo)^2*(2*z - 3*zexo + ztropo)
          )/(zexo - ztropo)^3
...
n_alt_index=[z=>clamp((i-1),1,length(alt)-2) for (i,z) in enumerate(alt)]
...
#get the total number density at this altitude
    ...
    sum([n_current[s][thisaltindex] for s in specieslist])
end
...
H2Oinitfrac=[H2Oinitfrac,fill(minimum(H2Osatfrac),length(alt)-2-length(H2Oinitfrac));]
...
clf()
plot(H2Oinitfrac/1e-6,alt[2:end-1]/1e5,color="blue",linewidth=2)
plot(detachedlayer/1e-6,alt[2:end-1]/1e5,color="blue",linewidth=2,linestyle="--")
...
#n_col(molecules/cm2)/(molecules/mol)*(gm/mol)*(1cc/gm)=(cm)*(10^4um/cm)=precipitable um
...
#effusion velocities need to be computed from the atmospheric
...
function Keddy(n_current,z)
    #eddy diffusion coefficient, stolen from Krasnopolsky
    #(1993). Scales as the inverse sqrt of atmospheric number density
...
thermaldiff(species) = get(Dict(:H=>-0.25,:H2=>-0.25,:HD=>-0.25,:D2=>-0.25,:He=>-0.25),species,0)
...
function scaleH(z,T::Float64,mm::Real)
    boltzmannK*T/(mm
                  *mH*marsM*bigG
                  )*(((z+radiusM)*1e-2)^2)*1e2#constants are in MKS. Convert to m and back to cm.
end
...
#again, the Any[] syntax might be removable once [[],[]] concatenation
...
#reactions where the supplied species is consumed.
    ...
    #reactions.
    losseqns=map(x->vcat(Any[network[x][1]...,network[x][3]]),
                 lhspos)#automatically finds a species where it occurs twice on the LHS
end
...
function production_rate(network,species)
    #return a symbolic expression for the loss rate of species in the
    #supplied reaction network.
    peqn=production_equations(network,species)#get the reactants and rate coefficients
    pval=:(+(#and add them all up
             $(map(x->:(*($(x...)))#take the product of each set of reactants and coeffecient
                   ,peqn)...)))
end
...
local_transport_rates=[[transpose([[symbol("t"*string(s)*"_up"),
                                    symbol("t"*string(s)*"_down"),
                                    symbol("t"*string(s)*"_above_down"),
                                    symbol("t"*string(s)*"_below_up")]
                                   for s in specieslist])...;]...;]
...
#coupled transport and chemical reaction network.
    ...
    while eps>1e-8
        ...
        fval=nthis-nstart-dt*ratefn(nthis,inactive,Jrates,T,M,tup,tdown,tlower,tupper)
        #if eps>1e-2; updatemat=chemJmat([nthis,nochems,phrates,T,M,dt]...); end;
        updatemat=chemJmat(nthis,inactive,Jrates,T,M,tup,tdown,tlower,tupper,dt)
...
function update!(n_current::Dict{Symbol,Array{Float64,1}},dt)
    ...
    tlower=Float64[[boundaryconditions(sp,dz,n_current)[1,:] for sp in specieslist]...;]
    tupper=Float64[[boundaryconditions(sp,dz,n_current)[2,:] for sp in specieslist]...;]
    
    #update to next timestep
    nthis=next_timestep(nstart,nthis,inactive,Jrates,T,M,tup,tdown,tlower,tupper,dt)
    nthismat=reshape(nthis,(length(activespecies),length(intaltgrid)))
...
function plotatm(n_current)
    ...
    for sp in fullspecieslist
        plot(n_current[sp],alt[2:end-1]/1e5,color=speciescolor[sp],linewidth=4)
    end
    ylim(0,200)
    xscale("log")
    xlim(1e-15,1e18)
...
#    global solarabs::Array{Array{Float64,1},1}
    solarabs=deepcopy(fill(fill(0.,size(solarflux,1)),length(alt)-2));
    #deepcopy keeps each row of solarabs independent (otherwise they
...
#    println(nlambda)
    ...
    for jspecies in Jratelist
        ...
        for ialt in [nalt:-1:1;]
            ...
            BLAS.axpy!(nlambda,
                       jcolumn,
                       crosssection[jspecies][ialt+1],
                       1,
                       solarabs[ialt],
                       1)
            ## println(string("At alt = ",alt[ialt+1],
            ##                ", max(x_",jspecies,") = ",maximum(crosssection[jspecies][ialt+1]),
            ##                ", max(solarabs) = ",maximum(solarabs[ialt,:])))
            ## readline(STDIN)
            ## for ilambda in [1:nlambda]
           ##     solarabs[ialt,ilambda]+=jcolumn*crosssection[jspecies][ialt+1][ilambda]
           ## end
        end
    end
    ...
    for j in Jratelist
        for ialt in [1:nalt;]
            n_current[j][ialt]=BLAS.dot(nlambda,
                                        solarabs[ialt],
                                        1,
                                        crosssection[j][ialt+1],
                                        1)
        end
    end
end
